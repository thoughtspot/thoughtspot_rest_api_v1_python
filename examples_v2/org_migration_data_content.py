...
def create_new_connection_from_primary(connection_name_to_replicate, new_connection_name, new_connection_password):
    ...
    orig_conn_config = conn_search_resp[0]["details"]["configuration"]
    orig_conn_config["password"] = new_connection_password
    # Must replace the password, it w
    conn_create_req = {
          "name": new_connection_name,
          "data_warehouse_type": conn_search_resp[0]["data_warehouse_type"],
...
def main():
    ...
    new_conn_id = create_new_connection_from_primary(connection_name_to_replicate=connection_name_to_replicate,
